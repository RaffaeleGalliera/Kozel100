...
public class EService {
  ...
  //Scrive tutto il "CONTENUTO" di un eccezione su un file di log FrontEnd
  public static void logFrontendException(Throwable exception,Hashtable info,Vector parameters) {
    
    StringBuffer parametersView=new StringBuffer(); //E' come una String, ma è manipolabile
    int i;
    
    String message=exception.getMessage(); //Salva il messaggio dell'Eccezione
    
    ByteArrayOutputStream stackTrace=new ByteArrayOutputStream();
    exception.printStackTrace(new PrintWriter(stackTrace,true)); //Scrive lo StackTrace nell' OutputStream stackTrace           
    
    for (i=0;i<parameters.size();i++) {
      parametersView.append( parameters.elementAt(i)+"\n" ); //Riempie la stringa parametersView con i parametri che stanno nel Vector parameters 
    }    
    
    //Utilizza la Classe di Servizio ErrorLog per scrivere l'errore su un file di log FrontEnd
    ErrorLog.FrontendErrorLog(message+"\n\n"+stackTrace.toString()+"\n\n"+info.toString()+"\n\n"+parametersView.toString());    
  }  
  
  
  //Scrive solo il MESSAGGIO di un eccezione su un file di log FrontEnd
